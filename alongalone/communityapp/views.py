...
def detail(request, blog_id):
    #1. blog_id번째 블로그 글을 데이터베이스로부터 갖고와서 pk가 blog_id인 블로그 객체를 가져옴.
...
def myprofile_fin(request):
    ...
    #photos_posts_6[6] = {}
    #id_posts_6[6] = {}
    #for i in range(0, len(posts_6)):
       #photos_posts_6[i] = posts_6[i].photo
       #id_posts_6[i] = posts_6[i].id
  
    #{'user_id' : user_id, 'current_posts' : current_posts}
    #user_id = get_object_or_404(User, pk = reqest.user.username)
    #user_author = get_object_or_404(User, pk = posts_all.author)
    #current_posts = Blog.objects.filter(author = user_id).order_by('-date')[:6]

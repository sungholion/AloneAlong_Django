# views.py : 해커톤 당일 기준 변동 있음 
# 4번라인 Comment 추가로 import
# 7번라인 CommentForm 삭제
...
########### honbabdetail 함수 변경(이걸로 휘영이랑 상의 후 바꾸기)
"""
def honbabdetail(request, blog_id):
    blog_detail = get_object_or_404(Blog, pk = blog_id) #특정 pk값을 가지는 객체 하나만 가져오기
    comment_all = Comment.objects.filter(post=blog_detail)
    print(comment_all)
    user_author = get_object_or_404(User, pk = blog_detail.author)
    current_posts = Blog.objects.filter(author = user_author.username).order_by('-date')[:3]
    #posts = Blog.objects.all() #블로그 객체를 모두 가져오는 코드
    #posts =Blog.objects.filter().order_by('-date')   #객체 필터링해서 가져오기 날짜 오름차순(date) 내림차순(-date)
    return render(request, 'honbabdetail.html', {'blog_detail' : blog_detail, 'user_author' : user_author, 'current_posts' : current_posts, 'comment_all':comment_all} )
"""
###########
...
def create_comment(request, blog_id):    
    if request.user.is_authenticated:
        ...
        comment.save()
        return redirect('honbabdetail', blog_id)

    else:
        return redirect("login")#로그인안된상태면 댓글 불허, login. html 렌더함
